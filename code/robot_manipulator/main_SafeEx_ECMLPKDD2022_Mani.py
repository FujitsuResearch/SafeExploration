...
header=''           # Header of saved models and results (default: empty)
...
class Mani_ECMLPKDD2022(gym.Env):
    ...
    def auto_u_adj_IFAC(self, u, step):
        q1,q2, q1dot,q2dot = self.state  # th := theta
...
def main():
    ...
    for method_ID in [0,1]: 
        ...
        ActorNet_name = header + "ActorNet_" + method +"_Mani.h5"
        CriticNet_name= header + "CriticNet_" + method + "_Mani.h5"
        ...
        for tri in range(num_trials):
            ...
            env = Mani_ECMLPKDD2022(mu_q1=mu_q1, sigma_q1=sigma_q1, mu_q2=mu_q2, sigma_q2=sigma_q2, 
                          mu_q1dot=mu_q1dot, sigma_q1dot=sigma_q1dot, mu_q2dot=mu_q2dot, sigma_q2dot=sigma_q2dot)
            if seed==-1:
                env.seed()
            else:
                env.seed(seed=tri)
            ...
            critic_optimizer = tf.keras.optimizers.Adam(lr_critic, decay=weight_decay)
            actor_optimizer = tf.keras.optimizers.Adam(lr_actor)            
            
            # Making the weights equal initially
            target_actor.set_weights(actor_model.get_weights())
            target_critic.set_weights(critic_model.get_weights())
            buffer = Buffer(num_states, num_actions, target_actor, target_critic, actor_model, critic_model,
                            actor_optimizer, critic_optimizer,
                            buffer_capacity=int(memory_capacity), batch_size=batch_size)


            res_total_reward.append([])
            res_total_reward_gd.append([])
            res_num_const_x.append(np.zeros(max_steps+1))
            ...
            for episode in range(max_episodes):
                ...
                for step in range(max_steps):
                    ...
                    if method == method_list[0]: 
                        action, u_type = env.auto_u_adj_IFAC(action, step)
                    elif method ==method_list[1]:
                        action, u_type = env.auto_u_adj(action, step)

                    state, reward, done, info, _,const_x = env.step(action,step)
                    res_num_const_x[tri][step+1]+=const_x

                    buffer.record((prev_state, action[0], reward, state))
                    episodic_reward += reward

                    buffer.learn()
                    update_target(target_actor.variables, actor_model.variables, tau_DDPG)
                    update_target(target_critic.variables, critic_model.variables, tau_DDPG)

                    prev_state = state

                res_total_reward[tri].append(episodic_reward)


                prev_state = env.reset(ini_state=ini_state)
                episodic_reward_gd = 0
            
                for step in range(max_steps):
                    tf_prev_state = tf.expand_dims(tf.convert_to_tensor(prev_state), 0)
                    action = policy(tf_prev_state, no_noise,actor_model)
                    state, reward_gd, done, info, q1q2 ,const_x = env.step(action,step)
                    episodic_reward_gd += reward_gd
                    prev_state = state

        
                res_total_reward_gd[tri].append(episodic_reward_gd)

                if (episode % logger_interval == 0) or (episode==max_episodes-1):
                    if episode==0:
                        print("trial:{0}".format(tri))
                    print("episode:{0}, total reward_gd:{1:.0f}".format(episode, episodic_reward_gd))

            if tri < num_trials-1:
                del actor_model, critic_model, target_actor, target_critic , buffer
                del critic_optimizer, actor_optimizer
            else:
                target_actor.save_weights(save_path +ActorNet_name)
                target_critic.save_weights(save_path +CriticNet_name)
                del actor_model, critic_model, target_actor, target_critic , buffer
                del critic_optimizer, actor_optimizer

                df_res_reward=pd.DataFrame(res_total_reward)
                df_res_reward.to_csv(save_path + header +'res_reward_' +method + '_Mani.csv')
                df_res_reward_gd=pd.DataFrame(res_total_reward_gd)
                df_res_reward_gd.to_csv(save_path + header +'res_reward_gd_' +method + '_Mani.csv')
                df_res_num_const_x=pd.DataFrame(res_num_const_x)
                df_res_num_const_x.to_csv(save_path + header +'res_num_const_x_' +method + '_Mani.csv')
