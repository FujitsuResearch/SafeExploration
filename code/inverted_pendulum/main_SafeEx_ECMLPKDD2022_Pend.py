...
class Pend_ECMLPKDD2022(gym.Env):
    ...
    def auto_u_adj_IFAC(self, u, step): # proposed by Okawa et al., 2020 in IFAC World Congress
        th, thdot = self.state  # th := theta
...
def main():
    ...
    for method_ID in [0,1]: 
        ...
        ActorNet_name =  header + "ActorNet_" + method +"_Pend.h5"
        CriticNet_name=  header + "CriticNet_" + method + "_Pend.h5"
        ...
        for tri in range( num_trials):
            ...
            for episode in range( max_episodes):
                ...
                for step in range( max_steps):
                    ...
                    state, reward, done, info, _,const_x = env.step(action)
                ...
                for step in range( max_steps):
                    tf_prev_state = tf.expand_dims(tf.convert_to_tensor(prev_state), 0)
                    action = policy(tf_prev_state, no_noise,actor_model)
                    state, reward_gd, done, info, th ,const_x = env.step(action)
            ...
            if tri <  num_trials-1:
                ...
                df_res_reward.to_csv( save_path +  header +'res_reward_' +method + '_Pend.csv')
                df_res_reward_gd=pd.DataFrame(res_total_reward_gd)
                df_res_reward_gd.to_csv( save_path +  header +'res_reward_gd_' +method + '_Pend.csv')
                df_res_num_const_x=pd.DataFrame(res_num_const_x)
                df_res_num_const_x.to_csv( save_path +  header +'res_num_const_x_' +method + '_Pend.csv')
